...
이 앱은 ChemDraw 전체 복제가 목적이 아닙니다. 수업 활동에 필요한 구조 입력, 구조 확인, 3D 관찰, 활동 결과 제출, 교사 피드백 반환을 안정적으로 제공하는 것이 목표입니다.
...
| 학생 활동 화면 | 수업코드 입력, 활동 선택, 예측, 구조 그리기, 제출 |
...
3. 오늘의 탐구 활동을 선택합니다. 교사가 서버 수업방에 활동을 제한한 경우 지정된 활동만 표시됩니다.
4. `예측하기`에서 예상 분자식, 예상 분자량, 구조를 그렇게 그린 이유를 작성합니다.
5. `분자 그리기`에서 분자 예시를 불러오거나 직접 구조를 그립니다.
6. `내 구조 확인하기`를 눌러 검증된 분자식과 평균 분자량을 확인합니다.
7. 참고 3D 구조와 입체 구조 예측을 관찰하고 차이를 비교합니다.
8. `정리하기`에서 활동 문항을 작성합니다.
9. `교사에게 제출하기`를 눌러 활동 결과를 제출합니다.
10. 교사가 피드백을 반환하면 같은 수업방에 다시 입장해 피드백을 확인합니다.
...
- [ ] 학생 제출이 서버 제출 목록에 표시된다.
- [ ] 교사가 AI 피드백 초안을 만들고 학생에게 반환할 수 있다.
