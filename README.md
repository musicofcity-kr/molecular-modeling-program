...
이 프로젝트는 학생이 `분자 선택 → 구조 만들기 → 구조 분석 → 3D 비교 →
생각 정리·제출`의 5단계를 자유롭게 오가며, 중심 원자와 전자 영역을 근거로
분자 구조를 설명하도록 돕는 것을 목표로 합니다.

> **2026-07-27 현재 범위:** 루트
> [`CODEX 분자구조모델링 메타프롬프트.md`](CODEX%20분자구조모델링%20메타프롬프트.md)의
> UX·QA 요구사항이 현재 기준입니다. 2026-07-12의 “단계 진행 UI가 없는 직접
> 작업대” 결정은 이력으로 보존하되 **superseded** 상태입니다. 현재 작업은
> 로컬 QA 95점을 통과한 미배포 release candidate이며, 이 문서는 production
> 배포 완료를 선언하지 않습니다.
...
위 이미지는 이전 UI 이력 참고용입니다. 현재 5단계 화면은
[`docs/qa-screenshots/final/`](docs/qa-screenshots/final/)의 데스크톱,
태블릿, 모바일·touch 완료 증거를 확인하세요. 최종 점수와 실행 결과는
[`QA_SCORECARD.md`](docs/QA_SCORECARD.md)와
[`QA_TEST_REPORT.md`](docs/QA_TEST_REPORT.md)에 있습니다.
...
- 단계 잠금 없이 이동하는 학생용 5단계 학습 흐름
- 데스크톱 진행 레일과 모바일 `선택·그리기·분석·3D·기록` 5탭
- Ketcher 학생용 `간편 모드`와 전체 도구를 쓰는 `고급 편집 모드`
- `VSEPR 근거표`와 참고 3D 구조/VSEPR 예상 모형 비교 화면
- 학생 판단·근거 작성, 교사 제출, 반환된 형성 피드백 확인
- 윤리 가이드 게이트, 개인정보처리방침, 이용약관 화면 포함
- Firebase Auth/Firestore 기반 수업방, 학생 제출, 교사 피드백 반환 흐름
- 교사용 AI 피드백 초안 생성 API 연결

현재 릴리즈 상태는 `unreleased release candidate / 로컬 QA 95점`입니다.
unit, build, E2E와 390×844 touch 자동화는 통과했지만 모바일 실기기,
학교 개인정보 검토 및 production URL 검증 전에는 배포 완료로 간주하지
않습니다.
...
`npm test`는 `*.emulator.test.ts`를 제외하므로 Firestore Security Rules는
반드시 `npm run test:firestore-rules`로 별도 실행합니다.

## 학생 사용 흐름

1. 윤리 가이드와 약관을 확인한 뒤 시작합니다.
2. `학생 활동`에서 교사가 안내한 수업코드, 입장 확인코드, 수업용 이름을 입력합니다.
3. 1단계 `분자 선택`에서 탐구할 분자를 고릅니다. 서버 수업방에 활동 템플릿이 지정된 경우 지정된 분자만 표시됩니다.
4. 2단계 `구조 만들기`에서 `예시 구조 불러오기`를 사용하거나 간편/고급 편집 모드로 직접 그립니다. 필요하면 `구조 초기화`를 사용합니다.
5. `2D 구조 분석하기`를 누릅니다. 3단계 `구조 분석`은 RDKit.js 검증이 성공한 경우에만 분자식과 VSEPR 근거를 표시합니다.
6. `VSEPR 근거표`에서 중심 원자, 결합/비공유/전체 전자 영역, 전자쌍 배열, 분자 구조와 예상 결합각을 확인합니다.
7. 4단계 `3D 비교`에 방문해 VSEPR 예상 모형과 좌표 기반 참고 3D 구조를 구분해 비교합니다. 현재 구조의 4단계 방문이 확인되어야 완료로 표시되고 제출 조건이 열립니다.
8. 5단계 `생각 정리·제출`에서 판단과 근거를 작성하고 `교사에게 제출하기`를 누른 뒤, 반환된 교사 피드백을 확인합니다.

5단계는 잠기지 않으며 데스크톱 진행 레일 또는 모바일 하단 5탭으로 자유롭게
이동합니다. 완료·검토 필요·오류 표시는 실제 검증 및 작성 상태를 반영합니다.
...
학생 제출은 구조 분석 성공, 생각 작성, Firebase 수업방 입장이 모두 완료된 경우에만
...
`다양한 분자의 분자구조 모델링`은 고등학교 화학 수업에서 학생이 2D 분자
구조를 만들고, 검증된 분자식과 VSEPR 근거를 확인한 뒤 참고 3D 자료와
교육용 예상 모형을 비교해 판단 근거를 작성하도록 돕는 웹앱입니다.

현재 학생 기본 화면은 단계 잠금이 없는 5단계 학습 흐름을 제공합니다.
...
| 학생 활동 화면 | 수업코드 입력 후 5단계 학습 흐름과 모바일 5탭 제공 |
| 분자 편집 영역 | Ketcher 기반 2D 입력, 간편/고급 편집 모드 |
| 구조 분석 결과 | RDKit.js 검증 기반 분자식과 핵심 학습 정보 표시 |
| VSEPR 근거표 | 중심 원자, 결합/비공유/전체 전자 영역, 배열, 구조, 예상 결합각 |
| 참고 3D 구조 | 정적 좌표 또는 PubChem SDF 기반 3Dmol.js 시각화 |
| 입체 구조 예상 | VSEPR 기반 교육용 예측 모형 |
| 생각 정리·제출 | 학생 판단과 근거 작성, 교사 제출, 반환 피드백 확인 |
...
3. 1단계에서 분자를 선택하고, 2단계에서 `예시 구조 불러오기` 또는 편집기로 구조를 만듭니다.
4. 필요에 따라 `간편 모드`와 `고급 편집 모드`를 전환합니다. 전환해도 편집 중 구조는 유지됩니다.
5. `2D 구조 분석하기`를 눌러 검증된 분자식과 VSEPR 근거표를 확인합니다.
6. 4단계에 방문해 참고 3D 구조와 VSEPR 예상 모형을 비교합니다. 구조를 바꾸면 이 방문 상태도 초기화됩니다.
7. 5단계에서 판단과 근거를 작성해 `교사에게 제출하기`를 누르고 반환 피드백을 확인합니다.

데스크톱에서는 5단계 진행 레일, 모바일에서는 `선택·그리기·분석·3D·기록`
5탭을 사용합니다. 단계 잠금은 없습니다.
...
| 기능 | 사용 방법 | 주의 |
|---|---|---|
| 예제 분자 불러오기 | 예제 목록에서 분자를 선택하고 `예시 구조 불러오기` 클릭 | 예제 데이터도 RDKit.js 검증 흐름을 통과한 값만 결과로 표시 |
| 직접 그리기 | Ketcher 편집기에서 원자, 결합, 고리 도구로 구조 작성 | 빈 구조나 원자가 오류가 있으면 결과값을 표시하지 않음 |
| 편집 모드 | 기본 `간편 모드` 또는 `고급 편집 모드` 선택 | 모드 전환 시 현재 구조 유지 |
| 구조 초기화 | `구조 초기화` 클릭 | 현재 편집 구조와 이전 분석 상태를 비움 |
| 구조 분석 | `2D 구조 분석하기` 클릭 | 분자식과 평균 분자량은 RDKit.js 결과 기준 |
| VSEPR 근거표 | 중심 원자와 전자 영역 수를 순서대로 확인 | RDKit 검증 이후에만 교육용 예측 표시 |
...
| 활동 결과 제출 | `교사에게 제출하기` 클릭 | trusted 서버 저장이 확인되어야 완료. 실패 시 현재 입력을 유지하고 재시도 |
| 교사 피드백 | `교사 피드백 확인하기` 클릭 | 같은 Firebase 수업방의 반환된 형성 피드백만 표시 |
...
| 구조 분석이 실패함 | 빈 구조, 끊어진 결합, 원자가 오류가 있는지 확인한 뒤 `2D 구조 분석하기`를 다시 누릅니다. |
...
- [ ] 데스크톱 5단계 진행 레일과 모바일 5탭이 표시되고 단계 잠금 없이 이동한다.
- [ ] 간편/고급 편집 모드를 전환해도 현재 구조가 유지된다.
- [ ] 예제 분자를 불러와 `2D 구조 분석하기`가 동작한다.
- [ ] 분자식과 평균 분자량이 RDKit.js 검증값으로 표시된다.
- [ ] VSEPR 근거표가 중심 원자와 전자 영역 수를 구분해 표시한다.
- [ ] 참고 3D 구조와 VSEPR 예상 모형이 별도 출처로 표시된다.
- [ ] 현재 구조로 4단계에 방문하기 전에는 제출이 잠기고, 방문 후에만 열린다.
- [ ] 생각 정리·제출 영역과 반환된 교사 피드백 영역이 표시된다.
- [ ] 학생 개인정보를 입력하지 않도록 수업 전에 안내한다.

## 배포 단계 원칙

1. GitHub `main` 브랜치를 Vercel production 기준으로 사용합니다.
2. Firebase Auth/Firestore 저장 기능은 teacher claim, trusted endpoint,
   security rules와 실제 project 설정을 함께 검증한 뒤에만 production에 연결합니다.
3. 앱은 학생 실명·학번·민감정보를 요구하지 않습니다. 학생에게 입력 자제를
   안내하고, AI provider 전송 사본은 명시적 개인정보 패턴을 삭제하며 교사가
   원본과 응답을 최종 확인합니다.
