...
| 학생 작업 화면 | 분자 선택, 구조 편집, 검증 결과, 3D/VSEPR 보기 |
...
| 모드 | 표시 내용 |
|---|---|
| 학생 모드 | 분자 선택, 구조 그리기, 검증 결과, 3D/VSEPR 보기 |
...
| 수업 활동 | 기본 학생 화면. 지정된 분자를 선택하고 구조 편집, 검증, 입체 구조 확인을 바로 수행 |

![수업 활동 모드](manual-assets/02-activity-mode.png)

## 5. 학생 기본 화면 사용법

학생 기본 화면은 단계 이동 없이 핵심 도구를 한 화면에 표시합니다.

1. `분자 선택`에서 작업할 분자를 고릅니다.
2. `구조 편집`에서 직접 구조를 그리거나 예제를 불러옵니다.
3. `내 구조 확인하기`를 누릅니다.
4. `구조 검증`에서 분자식과 평균 분자량을 확인합니다.
5. `구조 보기`에서 참고 3D 구조와 VSEPR 모형을 확인합니다.
6. 예상 입체 모형 아래 `나의 생각 정리`에 내용을 작성합니다.
7. `교사에게 제출하기`를 눌러 교사에게 보냅니다.

별도 예측 입력, 여러 성찰 문항, 7단계 진행 레일, 이전/다음 버튼, 결과 보고서 패널은 학생 기본 화면에 표시되지 않습니다. 생각 정리 입력은 예상 입체 모형 아래 한 곳만 제공합니다.
...
비교 모드는 교사용/고급 보기에서 RDKit.js 검증을 통과한 구조의 실제/외부 3D 좌표와 VSEPR 모형을 구분해서 보여줍니다.
...
4. 학생은 구조를 그리고 확인한 뒤 예상 입체 모형 아래 생각을 작성합니다.
5. 구조 검증, 생각 작성, 수업방 입장이 완료되면 `교사에게 제출하기`가 활성화됩니다.
6. 버튼을 누르면 현재 브라우저 제출함에 저장되고 Firestore 제출함에도 저장을 요청합니다.
...
2. 피드백은 점수나 등급이 아니라 형성 피드백으로 표시됩니다.
3. AI가 만든 초안도 교사가 확인한 뒤에만 학생에게 전달합니다.
