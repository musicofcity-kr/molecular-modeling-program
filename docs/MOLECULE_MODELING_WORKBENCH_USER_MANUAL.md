...
3. `서버 제출 목록 불러오기`를 눌러 학생 제출 자료를 확인합니다.
4. 제출 자료를 선택한 뒤 `AI 피드백 초안 만들기`를 누릅니다.
5. 서버의 `/api/create-feedback-draft`가 교사 권한과 수업방 배정을 확인한 뒤 피드백 초안을 만듭니다.
6. Vercel 환경변수 `OPENAI_API_KEY`가 있으면 OpenAI 호환 API로 초안을 만들고, 없거나 실패하면 로컬 가드레일 기반 초안을 만듭니다.
7. 교사는 초안을 읽고 필요한 문장을 수정합니다.
8. `교사 확인 후 학생에게 전달`을 누르면 Firestore 제출 문서에 피드백이 저장됩니다.
...
| AI 피드백 초안이 로컬 초안으로 표시됨 | Vercel 환경변수 `OPENAI_API_KEY` 또는 `AI_FEEDBACK_ENDPOINT`가 등록되어 있는지 확인합니다. |
