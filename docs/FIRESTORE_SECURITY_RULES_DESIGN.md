...
작성일: 2026-07-01  
상태: Firestore write 비활성, Firebase Auth 1단계 연결 후 보안 규칙 설계 유지

## 1. 목적

`다양한 분자의 분자구조 모델링` 앱에서 학생 활동 결과와 교사용 피드백을 Firestore에 저장하기 전에 권한 모델, 데이터 경계, Security Rules, 테스트 기준을 먼저 확정한다.

이번 단계에서는 실제 Firestore 저장을 켜지 않는다. 현재 `localStorage` 기반 임시 저장은 유지하고, 서버 저장은 Rules 테스트를 통과한 뒤 제한적으로 활성화한다.

2026-07-01 Auth 1단계에서는 Firebase Web SDK 초기화, 학생 Anonymous Auth,
교사용 Google/email 로그인을 연결했다. 그러나 teacher custom claim 검증,
trusted `joinClassroom`, 학생 멤버십 문서 생성, Firestore write는 아직
활성화하지 않았다.
...
- Firestore Security Rules가 완성되고 테스트되기 전까지 production 제출 저장은 비활성 상태로 둔다.
...
상태: SDK 초기화와 Google/email login UI 연결은 완료. teacher custom claim
검증과 교사용 Firestore 권한 연결은 다음 단계.
...
엔드포인트와 membership 문서 생성은 다음 단계.
